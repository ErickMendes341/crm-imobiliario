    ...
    if imovel_cli:
        st.markdown(
            ...
            <style>
                ...
                .price-tag {
                    background-color: #c59b27; color: white; padding: 6px 14px;
                    border-radius: 20px; font-weight: bold; font-size: 1.1rem; display: inline-block; margin-bottom: 15px;
                }
                .bairro-codigo { font-size: 0.85rem; color: #555555; margin-top: -10px; margin-bottom: 10px; line-height: 1.2; }
            </style>
            """,
        ...
        st.markdown(f'<div class="bairro-codigo">📍 {imovel_cli.get("bairro", "Passos-MG")} | Código: <b>{imovel_cli.get("codigo_imovel")}</b></div>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            if imovel_cli.get("area_terreno"): st.write(f"📐 **Lote:** {imovel_cli.get('area_terreno')} m²")
            if imovel_cli.get("area_construida"): st.write(f"🏗️ **Área Const.:** {imovel_cli.get('area_construida')} m²")
...
def gerar_descricao_ia(tipo, bairro, quartos, suites, vagas, valor):
    ...
    if not api_key: return "Erro: A chave 'GEMINI_API_KEY' não foi encontrada."
...
def calcular_total_matches(imoveis, leads):
    ...
    for l in leads_ativos:
        ...
        bairros = [str(b).strip().lower() for b in bairros_raw if str(b).strip()] if isinstance(bairros_raw, list) else []
...
def gerar_codigo_imovel_auto():
    ...
    if not imoveis: return "MS-001"
    numeros = []
    for item in imoveis:
        cod = item.get("codigo_imovel", "")
        if cod and ("-" in cod):
            try: numeros.append(int(cod.split("-")[1]))
...
st.markdown(f"""
    <style>
    .stApp {{ background-color: {COR_FUNDO_PAGINA}; color: {COR_TEXTO}; }}
    section[data-testid="stSidebar"] {{ background-color: {COR_AZUL_MARINHO} !important; }}
    section[data-testid="stSidebar"] * {{ color: #ffffff !important; }}
    .stCard {{
        background-color: {COR_CARD}; border-radius: 12px; padding: 14px;
        box-shadow: 0 4px 15px rgba(24, 30, 41, 0.06); margin-bottom: 12px;
        border-left: 6px solid {COR_DOURADO}; border: 1px solid #e2e8f0;
    }}
    .price-badge {{
...
with st.sidebar:
    ...
    menu = st.radio(
        ...
        ["📊 Dashboard", "📋 Imóveis Cadastrados", "📝 Novo Imóvel", "👤 Novo Lead", "👥 Funil de Leads", "🎯 Encontrar Matches", "📅 Visitas Agendadas"],
        index=0
    )
    st.divider()
...
if menu == "📊 Dashboard":
    ...
    with col1: st.metric(label="🏠 Disponíveis", value=len(imoveis_disponiveis))
    with col2: st.metric(label="👥 Leads Ativos", value=len(leads_em_negociacao))
    with col3: st.metric(label="🎯 Matches", value=total_matches)
    with col4: st.metric(label="📅 Visitas", value=len(visitas_pendentes))
...
# ==========================================
    ...
    with st.expander("🔍 **Filtros e Busca de Imóveis**", expanded=True):
        ...
        with f_col1: busca_codigo = st.text_input("🔎 Código", placeholder="Ex: MS-001")
        with f_col2: filtro_bairro_imovel = st.selectbox("📍 Bairro", ["Todos"] + BAIRROS_PASSOS)
        with f_col3: filtro_tipo = st.selectbox("🏠 Tipo", ["Todos"] + OPCOES_TIPO_IMOVEL)
        with f_col4: filtro_status = st.selectbox("📌 Status", ["Todos", "Disponível", "Vendido"], index=1)
        with f_col5:
    ...
    if busca_codigo: imoveis_filtrados = [i for i in imoveis_filtrados if busca_codigo.lower().strip() in i.get('codigo_imovel', '').lower()]
    if filtro_bairro_imovel != "Todos": imoveis_filtrados = [i for i in imoveis_filtrados if i.get('bairro') == filtro_bairro_imovel]
    if filtro_tipo != "Todos": imoveis_filtrados = [i for i in imoveis_filtrados if i.get('tipo') == filtro_tipo]
    if filtro_status != "Todos": imoveis_filtrados = [i for i in imoveis_filtrados if i.get('status', 'Disponível') == filtro_status]
    ...
    for imovel in imoveis_filtrados:
        ...
        with st.container():
            ...
            with col2:
                ...
                st.write(f"👤 **Proprietário:** {imovel.get('nome_proprietario', 'Não informado')} ({imovel.get('telefone_proprietario', '')})")
                ...
                novo_status = st.radio("Status do Imóvel:", ["Disponível", "Vendido"], index=0 if status_atual == "Disponível" else 1, key=f"status_imovel_{imovel_id}", horizontal=True)
...
# ==========================================
    ...
    st.write("Adicione fotos em lote ou tire várias fotos diretamente com a câmera do celular/computador. Todas receberão a marca d'água automaticamente.")
    st.divider()
    ...
    with c1:
        ...
        corretor_captacao = st.selectbox("Corretor Captação *", CORRETORES)
    with c2:
    ...
    if fotos_enviadas:
        ...
        for idx, foto_file in enumerate(fotos_enviadas):
            with cols_mini[idx % 4]:
                st.image(foto_file, caption=f"Foto {idx+1}", use_container_width=True)
    ...
    if st.button("💾 Salvar Imóvel Completo", use_container_width=True, type="primary"):
        if not nome_proprietario:
            st.warning("Por favor, preencha o nome do proprietário.")
        else:
            urls_fotos = []
            if fotos_enviadas:
                barra_progresso = st.progress(0)
                total_f = len(fotos_enviadas)
            ...
            try:
                ...
                limpar_cache()
                st.success(f"✅ Imóvel **{codigo_gerado}** cadastrado com sucesso com todas as fotos e marca d'água!")
...
# ==========================================
    ...
    with st.form("form_novo_lead", clear_on_submit=True):
        ...
        tipos_imovel = st.multiselect("Tipos de Imóvel*", OPCOES_TIPO_IMOVEL, default=["Casa", "Apartamento"])
        bairros_selecionados = st.multiselect("Bairros de Interesse", BAIRROS_PASSOS)
        observacoes = st.text_area("Observações")
        
        if st.form_submit_button("💾 Salvar Lead", type="primary", use_container_width=True):
...
# ==========================================
    ...
    tabs = st.tabs([f"{contagem_por_status[st]:02d} {st}" for st in STATUS_LEADS])
    for i, status_original in enumerate(STATUS_LEADS):
        with tabs[i]:
            for lead in leads_agrupados[status_original]:
                lead_id = lead.get('id')
                ...
                with st.expander(f"👤 **{nome_lead}** | 💰 R$ {orc_lead:,.2f}"):
                    st.write(f"📱 **WhatsApp:** {whatsapp_lead}")
                    idx_atual = STATUS_LEADS.index(status_original)
                    ...
                    if novo_st != status_original:
                        ...
                        limpar_cache()
                        st.rerun()
                    if st.button("🗑️ Excluir Lead", key=f"del_l_{lead_id}", type="secondary"):
...
# ==========================================
    ...
    for lead in leads_ativos:
        ...
        for im in imoveis_data:
            ...
            if orc_lead == 0.0 or preco_imovel <= (orc_lead * 1.15):
                matches_com_score.append((90, im))

        with st.expander(f"👤 **{nome_lead}** — 📱 {wsp_lead} | 🔥 {len(matches_com_score)} compatível(is)"):
            for prob, match in matches_com_score:
                cod_im = match.get('codigo_imovel')
                status_atual = interacoes_lead.get(cod_im, "⚪ Não Enviado")
                url_whatsapp = f"https://wa.me/{phone_clean}?text={urllib.parse.quote(f'Olá {nome_lead}, veja este imóvel compatível com seu perfil!')}"
                
                st.markdown(f"**Cód:** {cod_im} | **Tipo:** {match.get('tipo')} | **Valor:** R$ {float(match.get('valor_venda', 0)):,.2f}")
                st.link_button("📲 Enviar no WhatsApp", url_whatsapp, type="primary")
                
                novo_st_match = st.segmented_control("Status:", ["⚪ Não Enviado", "👁️ Visto", "❌ Sem Interesse", "📅 Visita Agendada"], default=status_atual if status_atual in ["⚪ Não Enviado", "👁️ Visto", "❌ Sem Interesse", "📅 Visita Agendada"] else "⚪ Não Enviado", key=f"seg_{lead_id}_{cod_im}")
                if novo_st_match != status_atual:
                    registrar_status_imovel_lead_direto(lead, cod_im, novo_st_match)
                    st.rerun()
...
# ==========================================
    ...
    for v in visitas_data:
        st.write(f"📅 **{v.get('data')} às {v.get('hora')}** | Imóvel: {v.get('imovel_info')} | Cliente: {v.get('lead_info')}")
        if st.button("🗑️ Cancelar Visita", key=f"del_vis_{v.get('id')}"):
